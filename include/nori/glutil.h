...
// Adapted from nanogui to use glad
...
public:
    ...
    void setUniform(const std::string &name, const Eigen::Matrix4f &mat, bool warn = true) {
    ...
    void setUniform(const std::string &name, const Eigen::Vector2f &v, bool warn = true) {
        glUniform2f(uniform(name, warn), v.x(), v.y());
    }

    /// Initialize a uniform parameter with a 3D vector
    void setUniform(const std::string &name, const Eigen::Vector3f &v, bool warn = true) {
        glUniform3f(uniform(name, warn), v.x(), v.y(), v.z());
    }

    /// Initialize a uniform parameter with a 4D vector
    void setUniform(const std::string &name, const Eigen::Vector4f &v, bool warn = true) {
...
struct Arcball {
    ...
    inline void setSize(Eigen::Vector2i size) { mSize = size; }
    inline const Eigen::Vector2i &size() const { return mSize; }
	...
	Eigen::Matrix4f matrix(const Eigen::Matrix4f &view) const {
...
extern Eigen::Vector3f project(const Eigen::Vector3f &obj, const Eigen::Matrix4f &model,
                        const Eigen::Matrix4f &proj, const Eigen::Vector2i &viewportSize);

extern Eigen::Vector3f unproject(const Eigen::Vector3f &win, const Eigen::Matrix4f &model,
                          const Eigen::Matrix4f &proj, const Eigen::Vector2i &viewportSize);

extern Eigen::Matrix4f lookAt(const Eigen::Vector3f &eye, const Eigen::Vector3f &center,
                       const Eigen::Vector3f &up);

extern Eigen::Matrix4f ortho(const float left, const float right, const float bottom,
                      const float top, const float zNear, const float zFar);

extern Eigen::Matrix4f frustum(const float left, const float right, const float bottom,
                        const float top, const float nearVal,
                        const float farVal);

extern Eigen::Matrix4f scale(const Eigen::Matrix4f &m, const Eigen::Vector3f &v);

extern Eigen::Matrix4f translate(const Eigen::Matrix4f &m, const Eigen::Vector3f &v);
